...
http.createServer((req, res) => { res.end('WinGo Betting Engine Active'); }).listen(process.env.PORT || 8080);
...
async function callApi(endpoint, payload, authToken = null) {
    ...
    const headers = {
        ...
        "Authorization": authToken || "", // Token ကို Bearer ပါပြီးသားအဖြစ် ထည့်မည်
...
// --- 💰 လက်ကျန်ငွေစစ်ဆေးခြင်း ---
async function getBalance(chatId) {
    // ပုံထဲက Endpoint အတိုင်း GetBalance ကို သုံးထားပါတယ်
...
bot.on('callback_query', async (query) => {
    ...
    bot.sendMessage(chatId, `💰 လက်ကျန်ငွေ: **${balance}** MMK\n🏦 **${side === "Big" ? "အကြီး" : "အသေး"}** အတွက် ထိုးမည့်ပမာဏ (10 ကနေစ၍) ရိုက်ထည့်ပါ:`);
});
...
bot.on('message', async (msg) => {
    ...
    if (user_db[chatId].pendingSide && /^\d+$/.test(msg.text)) {
        ...
        // --- 📊 Auto BetCount Calculation (Base 10) ---
        const baseAmount = 10;
        ...
        if (res && res.msgCode === 0) {
            bot.sendMessage(chatId, `✅ **${side === "Big" ? "အကြီး" : "အသေး"}** မှာ **${totalAmount}** ဖိုး ထိုးပြီးပါပြီ!\n(Count: ${calculatedBetCount})\nလက်ကျန်ငွေ: ${res.data.amount || "Updating..."}`);
        } else {
            bot.sendMessage(chatId, `❌ ထိုး၍မရပါ။ အမှား: ${res?.message || "Network Error"}`);
        }
    ...
    if (user_db[chatId].tempPhone && !user_db[chatId].token) {
        ...
        if (res?.msgCode === 0) {
            // Token ကို Bearer format အမှန်ဖြစ်အောင် သိမ်းပါမည်
            user_db[chatId].token = "Bearer " + res.data.tokenHeader + res.data.token;
            return bot.sendMessage(chatId, "✅ Login အောင်မြင်သည်။", { reply_markup: { keyboard: [["🚀 ၃၀ စက္ကန့် စတင်ရန်", "🚀 ၁ မိနစ် စတင်ရန်"], ["📈 နိုင်/ရှုံး (၅၀) မှတ်တမ်း", "🛑 AI ကို ရပ်တန့်ရန်"]], resize_keyboard: true } });
        }
    }
