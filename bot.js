...
// Render Alive Fix (Render ပေါ်မှာ Bot မအိပ်အောင် လုပ်ပေးထားတာပါ)
http.createServer((req, res) => { res.end('WinGo v81: Final Manual Flow'); }).listen(process.env.PORT || 8080);

// --- 🔑 BOT TOKEN (ဒီမှာ ထည့်ပေးထားပါတယ်) ---
...
// --- 🛡️ Security Logic (API Signatures) ---
...
// --- 🧠 AI Intelligence (Dragon & Mirror Logic) ---
function runAI(history) {
    ...
    // Dragon 3 ပွဲထက်ကျော်ရင် အမြီးလိုက်မယ်၊ မဟုတ်ရင် ပြောင်းပြန်လိုက်မယ်
    ...
    while (user_db[chatId]?.running) {
        ...
        if (res?.msgCode === 0 && res.data?.list?.length > 0) {
            ...
            if (lastRound.issueNumber !== data.last_issue) {
                ...
                // VIP Report (အရင်ပွဲ နိုင်/ရှုံး ပြခြင်း)
                if (data.last_pred) {
                    ...
                    // AI History ထဲ ပေါင်းထည့်ခြင်း (ပွဲ ၅၀ အထိ)
                ...
                // AI New Signal (အသစ်တွက်ချက်ခြင်း)
...
bot.on('message', async (msg) => {
    ...
    // Amount ရိုက်ရင် တန်းထိုးပေးမယ့်အပိုင်း
